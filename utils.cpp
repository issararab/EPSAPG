...
void psiBlast(int i, string dbsize, string use_sw_tback, string output_profile,
	...
	string cmd = "psiblast -query isar.q." + to_string(i) + " -db isar.db." + 
					to_string(i) + " -evalue 10";
	if (!dbsize.empty())
		cmd += " -dbsize " + dbsize;
...
void help(bool detailed){
	cout << " ++USAGE \n"
	 << "  epsapg [-h] [-help] [-query input_file] \n"
	 ...
	 << "\n ++DESCRIPTION \n"
	 << "      EPSAPG: An Extensive Protein Sequence Alignment Profiles Generator Combining \n"
	 << "      MMseqs2 and PSI-BLAST in a Single Pipeline. It can generate both profiles and PSSMs.)\n"
	 << endl;
	if(!detailed)
		return;
	cout << "\n ++MANDATORY PARAMETERS \n"
     ...
     <<	"\n   MMseqs2 output result. For accurate statistics of the profile, provide "
     ...
     <<	"\n   Enable or disable outputing alignment profile of a query sequence against a database."
     <<	"\n   If disabled, it will print on console. (enabled by default)\n"
     <<	"\n -output_pssm <boolean[0|1]>"
     <<	"\n    Enable or disable outputing position-specific scoring matrix checkpoint file"
     <<	"\n   (disabled by default)\n"
     <<	"\n -output_ascii_pssm <boolean[0|1]>"
     <<	"\n    Enable or disable outputing ASCII version of position-specific scoring matrix"
     <<	"\n   checkpoint file (disabled by default)"
     <<	"\n"
     <<	"\n"
     << endl;
}